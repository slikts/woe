...
function draw(dt) {
  ...
  var _ents = Object.keys(ents).map(function(key) {
    var xq = ents[key];
    xq.coords = xq._x + '_' + xq._y + '_' + xq._z;
    return xq;
  });

  _ents = _ents
    .sort(function(a, b) {
      return a.coords < b.coords ? -1 : 1;
    });
  ...
  if (!avatar || !changes || document.hidden) {
    next();
  ...
  _ents.forEach(function(ent, i) {
    ...
    if (ent.type === 'avatar') {
      var p2d = p3d.toGlobalCoordinates();
      labels.push({
        x: p2d.x + canvas.width / 2,
        y: p2d.y + canvas.width / 2,
        text: ent._id
      });
    }
    // var p2dd = p3d.toGlobalCoordinates();
    // plabels.push({
    //   _x: p2dd.x + canvas.width / 2,
    //   _y: p2dd.y + canvas.height / 2,
    //   x: ent._x,
    //   y: ent._y,
    //   z: ent._z,
    //   i: i,
  ...
  // plabels.forEach(function(plab) {
  //   ctx.fillText(plab.i + ': ' + (plab.x) + ', ' + (plab.y) + ', ' + (plab.z), plab._x, plab._y);
  //   // ctx.fillText(plab.i + ': ' + plab.t, plab.x, plab.y + 10);
  // });
  labels.forEach(function(lab) {
    // ctx.fillStyle = 'Red';
    ctx.fillText(lab.text, lab.x, lab.y - 43);
  });
